...
class my_loader_LRN_f2_rand2(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    if self.transform1 is not None:
      ...
      x1 = np.random.randint(0, 1024 - 384 -1)
      y1 = np.random.randint(0, 1024 - 384 - 1)
      #x1 = np.random.randint(0, 1536 - 384 -1)
      #y1 = np.random.randint(0, 1536 - 384 - 1)
      # print(x1,y1)
      imgC = self.transform6(imgA)
      imgD = self.transform6(imgB)
      imgA, imgB = self.transform4(x1, y1, imgA, imgB)
...
#TODO
class my_loader_train(data.Dataset):
  def __init__(self, root, transform=None, loader=default_loader2, seed=None, pre="", label_file=''):
    self.source_path = root+os.sep+pre+"source"
    #self.target_path = root+os.sep+pre+"target"
    self.target_path = root+os.sep+pre+"result"
    src_imgs = make_dataset(self.source_path)
    self.label_file = label_file
    if label_file!='':
        self.labels_0 = make_labels(label_file, 0)
  ...
  def __getitem__(self, index):
    ...
    (short_name, extension) = os.path.splitext(tempfilename)
    tmp = short_name.split('_')
    tar_path = self.target_path + os.sep + 'res' + '_' + tmp[1] + '.png'
    imgA = self.loader(src_path)
    imgB = self.loader(tar_path)
    if self.label_file!='':
        label_0 = self.labels_0[tempfilename]
...
#TODO
class my_loader_LCDmoire(data.Dataset):
  def __init__(self, root, transform=None, loader=default_loader2, seed=None, pre="", label_file=''):
    self.source_path = root+os.sep+pre+"moire"
    self.target_path = root+os.sep+pre+"clear"
    src_imgs = make_dataset(self.source_path)
    if len(src_imgs) == 0:
      raise(RuntimeError("Found 0 images in subfolders of: " + root + "\n"
                 "Supported image extensions are: " + ",".join(IMG_EXTENSIONS)))
  ...
  def __getitem__(self, index):
    ...
    (filename, tempfilename) = os.path.split(src_path)
    (short_name, extension) = os.path.splitext(tempfilename)
    tar_path = self.target_path + os.sep + short_name + '.jpg'
    imgA = self.loader(src_path)
    imgB = self.loader(tar_path)
...
class my_loader_CFAmoire(data.Dataset):
  def __init__(self, root, transform=None, loader=default_loader2, seed=None, pre="", label_file=''):
    self.source_path = root+os.sep+pre+"input"
    self.target_path = root+os.sep+pre+"gt"
    src_imgs = make_dataset(self.source_path)
    if len(src_imgs) == 0:
      raise(RuntimeError("Found 0 images in subfolders of: " + root + "\n"
                 "Supported image extensions are: " + ",".join(IMG_EXTENSIONS)))
  ...
  def __getitem__(self, index):
    ...
    (short_name, extension) = os.path.splitext(tempfilename)
    tmp = short_name.split('_')
    tar_path = self.target_path + os.sep + tmp[0] + '_gt.png'
    imgA = self.loader(src_path)
    imgB = self.loader(tar_path)
...
#TODO
class my_loader_mopnet(data.Dataset):
...
class my_loader_LRN(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    # TODO
    tar_path = self.target_path + os.sep + 'tar' + '_' + tmp[1] + '.png'
    #tar_path = self.target_path + os.sep + 'tar_001.png'
    imgA = self.loader(src_path)
    #isFile = os.path.isfile(tar_path)
    #if not isFile:
    #    imgB = None
    #    #imgB = np.zeros((1920,1080))
    #else:
    imgB = self.loader(tar_path)
    if self.label_file!='':
        label_0 = self.labels_0[tempfilename]
